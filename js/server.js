...
*/
let felhasznalo=localStorage.getItem("vasarloNev");
let felhasznaloId=localStorage.getItem("vasarloId");
  ...
  xmlhttp.onreadystatechange = function() {
    if(xmlhttp.readyState==4 && xmlhttp.status==200){
      ...
      for(let i=0;i<object.length;i++){
        if(object[i].vasarloId == felhasznaloId){
          document.getElementById("vasarloNev").innerHTML=felhasznalo;
        }
      }
